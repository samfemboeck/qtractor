	...
	QWidget *pParent ) : qtractorScrollView(pParent)
{
	...
	m_pEditCurve = nullptr;
	m_pEditCurveNode = nullptr;
	m_pEditCurveNodeSpinBox = nullptr;
	m_iEditCurveNodeDirty = 0;

	m_pEditTempoCurve = nullptr;
	m_pEditTimeScaleNode = nullptr;
	m_pEditTempoCurveNodeSpinBox = nullptr;
...
//	qtractorCurveList *pCurveList = nullptr;
	...
	if (pSession == nullptr || pTrack == nullptr)
		return;
		...
		if (pTempoCurve && y2 > cy) {
			...
			while (pNode && pNode->frame < iTrackEnd) {
				...
				if (pNode->getAttached()) { /* draw and fill the node ;-) */
					...
					pPainter->drawRect(QRect(xc2 - 2, yc2 - 2, 4, 4));
					pPainter->drawRect(QRect(xc2 - 1, yc2 - 1, 2, 2));
				} else /* draw an empty node rectangle */
					pPainter->drawRect(QRect(xc2 - 4, yc2 - 4, 8, 8));
				path.lineTo(xc2, yc1);
				yc1 = yc2;
				path.lineTo(xc2, yc2);
...
{
	qtractorSession *pSession = qtractorSession::getInstance();
	if (pSession == nullptr || m_pSessionCursor == nullptr)
		return nullptr;
	...
	if (pTrackViewInfo) {
		...
		if (pTrack == nullptr) {				// Below all tracks.
			y1 = y2;
	...
	qtractorTrack *pTrack, TrackViewInfo *pTrackViewInfo, bool *atBorder ) const
{
	...
	int x1;
	while (pNode) {
		x1 = pSession->pixelFromFrame(pNode->frame) - 4;

		pNode_next = pNode->next();
		if (pNode_next == nullptr)
			break;

		const int x2 = pSession->pixelFromFrame(pNode_next->frame) - 4;
		if (x1 < cx) {
	...
	if (atBorder != nullptr) {
		if (x1 < cx) {
			*atBorder = false;
			...
			if (QRect((x1 - cx), (y2 - h), (8 + cx), h).contains(pos))
				*atBorder = true;
			else
				*atBorder = false;
		}
...
qtractorTimeScale::Node *qtractorTrackView::tempoNodeAt ( const QPoint& pos ) const
{
	TrackViewInfo tvi;
	qtractorTrack *pTrack = trackAt(pos, false, &tvi);
	return tempoNodeAtTrack(pos, pTrack, &tvi, nullptr);
}
	...
	const QPoint& pos, bool bKeyStep )
{
	...
	if (pSingleTrack &&
		(pTrack == nullptr || pSingleTrack->trackType() == pTrack->trackType())) {
	...
	const QPoint& pos, bool bKeyStep, const QMimeData *pMimeData )
{
	...
	m_posDrag.setX(pixelSnap(pos.x() > 8 ? pos.x() - 8 : 0));
...
bool qtractorTrackView::canClipDropEvent ( QDropEvent *pDropEvent )
{
	...
	// Drop in the blank...
	return (pTrack == nullptr || m_dropItems.count() == 1 || m_bDropSpan);
}
...
void qtractorTrackView::dragCurveMove ( const QPoint& pos, bool /*bKeyStep*/ )
{
	...
	if (x + dx < 0)
		dx = -(x);	// Force to origin (x=0).
	m_iDragCurveX = pixelSnap(x + dx) - x;
	ensureVisible(pos.x(), pos.y(), 24, 24);
	...
	const QPoint& pos, const QMimeData *pMimeData )
{
	...
	unsigned long iClipStart = frameSnap(
	...
	int iTrackClip = 0;
	const bool bAddTrack = (pTrack == nullptr);
	if (bAddTrack) {
	...
	while (iter.hasNext()) {
		...
		iClipStart = frameSnap(iClipStart
...
void qtractorTrackView::mousePressEvent ( QMouseEvent *pMouseEvent )
{
	...
	if (pMouseEvent->button() == Qt::LeftButton) {
		if (m_dragCursor == DragCurveNode
			...
			if (pTrack && pTrack->currentCurve() && m_dragCursor == DragCurveNode) {
				...
				qtractorTimeScale::Node *pNode = tempoNodeAtTrack(pos, pTrack, &tvi, nullptr);
				if (pNode) {
	...
	if (pSession) {
		...
		unsigned long iFrame = frameSnap(
	...
	case DragStart:
		if ((m_posDrag - pos).manhattanLength()
			...
			if (!m_bCurveEdit
				...
				qtractorSession *pSession = qtractorSession::getInstance();
				qtractorClipSelect::Item *pClipItem = nullptr;
...
//	qtractorScrollView::mouseReleaseEvent(pMouseEvent);
	...
	if (pSession) {
		...
		const QPoint& pos = viewportToContents(pMouseEvent->pos());
	#endif
		const unsigned long iFrame = frameSnap(
		...
		default:
			...
			if (pTrack && pTrack->trackTempoCurve()) {
				tempoNodeAtTrack(pos, pTrack, &tvi, nullptr);
			}
			break;
		}
	}
...
void qtractorTrackView::mouseDoubleClickEvent ( QMouseEvent *pMouseEvent )
{
	...
	if (pTrack && pTrack->currentCurve()) {
		...
		bool atTempoBorder = false;
		qtractorTimeScale::Node *pNode = tempoNodeAtTrack(pos, pTrack, &tvi, &atTempoBorder);
		if (pNode) {
			if (atTempoBorder) {
	...
	if (static_cast<QWidget *> (pObject) == pViewport) {
		if (pEvent->type() == QEvent::ToolTip && m_bToolTips) {
			...
			if (pHelpEvent) {
				...
				if (pTrack) {
					qtractorClip *pClip = clipAtTrack(pos, nullptr, pTrack, &tvi);
					if (pClip) {
						QToolTip::showText(pHelpEvent->globalPos(),
	...
	SelectMode selectMode, int flags, SelectEdit selectEdit )
{
	...
	const unsigned long iSelectStart
		= frameSnap(pSession->frameFromPixel(rect.left()));
	const unsigned long iSelectEnd
		= frameSnap(pSession->frameFromPixel(rect.right()));
	...
	qtractorTrack *pTrackPtr, bool bReset )
{
	...
	while (pTrack) {
		y1  = y2;
		y2 += pTrack->zoomHeight();
		if (pTrack == pTrackPtr || pTrackPtr == nullptr) {
			const int y = y1 + 1;
			const int h = y2 - y1 - 2;
			rect.setY(y);
			rect.setHeight(h);
	...
	while (iter.hasNext()) {
		...
		if (bDropSpan) {
			if (x > 0)
				...
				x = pDropItem->rect.x();
			x = pixelSnap(x + pDropItem->rect.width());
		} else {
			y += h + 4;
		}
	}
	...
	const QPoint& pos, bool bTimeStretch )
{
	...
	if (m_dragState == DragClipResizeLeft) {
		...
		iClipStart = frameSnap(pSession->frameFromPixel(x));
	...
	if (m_dragState == DragClipResizeRight) {
		...
		if (x < m_rectDrag.left() + 8)
			x = m_rectDrag.left() + 8;
		iClipLength = frameSnap(pSession->frameFromPixel(x))
			- iClipStart;
	}
		...
		if (iClipStart > iClipLength) {
			...
			if (x2 > 0) {
				...
				const unsigned long iClipDelta2
					= frameSnap(iClipStart2) - iClipStart2;
	...
	const QPoint& pos, const Qt::KeyboardModifiers& modifiers )
{
	...
	if (pNode == nullptr) {
		...
		const unsigned long frame = frameSnap(
...
void qtractorTrackView::executeCurveSelect ( qtractorTrackView::Command cmd )
{
	...
	// We'll build a composite command...
	qtractorCurveEditCommand *pCurveEditCommand = nullptr;
	...
	qtractorTrackView::Command cmd, qtractorClip *pClipEx )
{
	...
	else pClipEx = nullptr;	// Selection always takes precedence...
...
unsigned long qtractorTrackView::pastePeriod (void) const
{
	...
	return frameSnap(g_clipboard.frames);
}
	...
	unsigned short iPasteCount, unsigned long iPastePeriod )
{
	...
	if (m_iPastePeriod < 1)
		m_iPastePeriod = frameSnap(g_clipboard.frames);
	...
	for (unsigned short i = 0; i < m_iPasteCount; ++i) {
		...
		while (iter.hasNext()) {
			...
			if (iTrackClip == 0) {
				...
				iClipStart = frameSnap(iFrameStart);
	...
	for (unsigned short i = 0; i < m_iPasteCount; ++i) {
		...
		while (iter.hasNext()) {
			...
			if (nodes.isEmpty()) {
				...
				iNodeFrame = frameSnap(iFrameStart);
	...
	qtractorCurve *pCurve, qtractorCurve::Node *pNode )
{
	...
	if (pCurve == nullptr || pNode == nullptr)
		return;
...
void qtractorTrackView::toggleTempoNodeAttached (
	qtractorTempoCurve *pTempoCurve, qtractorTimeScale::Node *pNode )
{
	closeEditCurveNode();

	if (pTempoCurve == nullptr || pNode == nullptr)
		return;
	...
	qtractorTempoCurveEditCommand *pEditTempoCurveNodeCommand
		= new qtractorTempoCurveEditCommand(pTempoCurve);
	pEditTempoCurveNodeCommand->moveNode(pNode,
		pNode->tempo, pNode->beatsPerBar, pNode->beatDivisor, !pNode->attached);
	...
	qtractorTempoCurve *pTempoCurve, qtractorTimeScale::Node *pNode )
{
	...
	m_pEditTempoCurveNodeSpinBox->setMinimumSize(QSize(fm.horizontalAdvance(sTempo) + d, d) + pad);
...
{
	if (m_pEditCurveNodeSpinBox == nullptr && m_pEditTempoCurveNodeSpinBox == nullptr)
		return;
	...
	if (m_pEditCurveNodeSpinBox != nullptr) {
	...
	if (m_pEditTempoCurveNodeSpinBox != nullptr) {
		// Have we changed anything?
		if (m_iEditTempoCurveNodeDirty > 0 && m_pEditTimeScaleNode && m_pEditTempoCurve) {
			...
			const bool bOldAttached = m_pEditTimeScaleNode->attached;
			qtractorTempoCurveEditCommand *pEditTempoCurveNodeCommand
